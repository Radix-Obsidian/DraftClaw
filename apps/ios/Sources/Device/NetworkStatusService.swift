...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> DraftClawNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> DraftClawNetworkStatusPayload {
        let status: DraftClawNetworkPathStatus = switch path.status {
        ...
        var interfaces: [DraftClawNetworkInterfaceType] = []
        if path.usesInterfaceType(.wifi) { interfaces.append(.wifi) }
        if path.usesInterfaceType(.cellular) { interfaces.append(.cellular) }
        if path.usesInterfaceType(.wiredEthernet) { interfaces.append(.wired) }
        if interfaces.isEmpty { interfaces.append(.other) }

        return DraftClawNetworkStatusPayload(
            status: status,
    ...
    private static func fallbackPayload() -> DraftClawNetworkStatusPayload {
        DraftClawNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
