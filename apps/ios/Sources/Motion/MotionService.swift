...
final class MotionService: MotionServicing {
    func activities(params: DraftClawMotionActivityParams) async throws -> DraftClawMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[DraftClawMotionActivityEntry], Error>) in
        ...
        return DraftClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: DraftClawPedometerParams) async throws -> DraftClawPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<DraftClawPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = DraftClawPedometerPayload(
