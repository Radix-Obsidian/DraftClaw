...
struct IOSGatewayChatTransport: DraftClawChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> DraftClawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(DraftClawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> DraftClawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(DraftClawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [DraftClawChatAttachmentPayload]) async throws -> DraftClawChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [DraftClawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(DraftClawChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(DraftClawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: DraftClawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: DraftClawChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: DraftClawAgentEventPayload.self)
                        {
