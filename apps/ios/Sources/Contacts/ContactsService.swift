...
final class ContactsService: ContactsServicing {
    ...
    func search(params: DraftClawContactsSearchParams) async throws -> DraftClawContactsSearchPayload {
        ...
        return DraftClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: DraftClawContactsAddParams) async throws -> DraftClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return DraftClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return DraftClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> DraftClawContactPayload {
