...
final class RemindersService: RemindersServicing {
    func list(params: DraftClawRemindersListParams) async throws -> DraftClawRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[DraftClawReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return DraftClawReminderPayload(
        ...
        return DraftClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: DraftClawRemindersAddParams) async throws -> DraftClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = DraftClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return DraftClawRemindersAddPayload(reminder: payload)
    }
