...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: DraftClawProtocol.AnyCodable(["ok": DraftClawProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: DraftClawProtocol.AnyCodable(["ok": DraftClawProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = DraftClawProtocol.AnyCodable([
            "runId": DraftClawProtocol.AnyCodable("run-1"),
            "sessionKey": DraftClawProtocol.AnyCodable("main"),
            "state": DraftClawProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: DraftClawProtocol.AnyCodable(["a": DraftClawProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
