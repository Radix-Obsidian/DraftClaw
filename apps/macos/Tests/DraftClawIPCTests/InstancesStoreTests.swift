...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: DraftClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: DraftClawProtocol.AnyCodable] = [
            "presence": .init([DraftClawProtocol.AnyCodable(entry)]),
        ]
        let payload = DraftClawProtocol.AnyCodable(payloadMap)
