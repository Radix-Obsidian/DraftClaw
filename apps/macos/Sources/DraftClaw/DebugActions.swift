...
enum DebugActions {
    private static let verboseDefaultsKey = "draftclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "draftclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = DraftClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "DraftClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = DraftClawPaths.configURL
        guard
