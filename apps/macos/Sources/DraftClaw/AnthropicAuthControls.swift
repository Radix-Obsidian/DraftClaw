...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: DraftClawOAuthStore.AnthropicOAuthStatus = DraftClawOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: DraftClawOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = DraftClawOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = DraftClawOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try DraftClawOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. DraftClaw can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: DraftClawOAuthStore.AnthropicOAuthStatus,
