...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.draftclaw/draftclaw.json. Prefer DRAFTCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.draftclaw/draftclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.draftclaw/draftclaw.json. Restart the gateway to apply."
        }
