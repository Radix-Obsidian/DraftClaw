...
enum CommandResolver {
    private static let projectRootDefaultsKey = "draftclaw.gatewayProjectRootPath"
    private static let helperName = "draftclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let draftclawEntry = root.appendingPathComponent("draftclaw.mjs").path
        if FileManager().isReadableFile(atPath: draftclawEntry) { return draftclawEntry }
        let binEntry = root.appendingPathComponent("bin/draftclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/draftclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let draftclawPaths = self.draftclawManagedPaths(home: home)
        if !draftclawPaths.isEmpty {
            extras.insert(contentsOf: draftclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + draftclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func draftclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".draftclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func draftclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectDraftClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("draftclaw.mjs").path,
            root.appendingPathComponent("bin/draftclaw.js").path,
        ]
    ...
    static func hasAnyDraftClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.draftclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func draftclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let draftclawPath = self.projectDraftClawExecutable(projectRoot: root) {
                return [draftclawPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "draftclaw", subcommand] + extraArgs
            }
            if let draftclawPath = self.draftclawExecutable(searchPaths: searchPaths) {
                return [draftclawPath, subcommand] + extraArgs
            }

            let missingEntry = """
            draftclaw entrypoint missing (looked for dist/index.js or draftclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real draftclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/draftclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v draftclaw >/dev/null 2>&1; then
          CLI="$(command -v draftclaw)"
          draftclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/draftclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/draftclaw.mjs"
            node "$PRJ/draftclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/draftclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/draftclaw.js"
            node "$PRJ/bin/draftclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent draftclaw"
          pnpm --silent draftclaw \(quotedArgs);
        else
          echo "draftclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? DraftClawConfigFile.loadDict()
