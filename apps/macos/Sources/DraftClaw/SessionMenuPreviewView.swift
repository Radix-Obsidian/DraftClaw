...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.draftclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> DraftClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: DraftClawSessionPreviewEntry,
    ...
    private static func cache(payload: DraftClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: DraftClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [DraftClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [DraftClawKit.AnyCodable]) -> [DraftClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(DraftClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: DraftClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: DraftClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: DraftClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: DraftClawChatMessage) -> String? {
