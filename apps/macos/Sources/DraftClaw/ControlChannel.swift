...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: DraftClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.draftclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: DraftClawKit.AnyCodable]()) {
                $0[$1.key] = DraftClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: DraftClawProtocol.AnyCodable?) -> [String: DraftClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: DraftClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: DraftClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: DraftClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("draftclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("draftclaw.control.agent")
}
