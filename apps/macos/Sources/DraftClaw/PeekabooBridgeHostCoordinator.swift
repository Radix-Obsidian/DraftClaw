...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.draftclaw", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: DraftClawPeekabooBridgeServices?
    private static var draftclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("DraftClaw", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = DraftClawPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.draftclawSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.draftclawSocketPath, privacy: .public)")
    }
...
@MainActor
private final class DraftClawPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.draftclaw.peekaboo")
