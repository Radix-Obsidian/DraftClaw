...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (DraftClaw token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: DraftClawOAuthStore.AnthropicOAuthStatus = DraftClawOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.draftclaw", category: "anthropic-oauth")
...
enum DraftClawOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let draftclawOAuthDirEnv = "DRAFTCLAW_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "DraftClaw OAuth token file not found"
            case .unreadableFile: "DraftClaw OAuth token file not readable"
            case .invalidJSON: "DraftClaw OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in DraftClaw OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "DraftClaw OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.draftclawOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".draftclaw", isDirectory: true)
