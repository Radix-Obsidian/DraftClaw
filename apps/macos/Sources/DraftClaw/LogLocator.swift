...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["DRAFTCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/draftclaw")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("draftclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("draftclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/draftclaw/ (rolling or stdout), or nil if none exist.
