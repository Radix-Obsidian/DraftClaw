...
enum DraftClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = DraftClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = DraftClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        DraftClawLogging.bootstrapIfNeeded()
        let label = DraftClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct DraftClawOSLogHandler: LogHandler {
...
struct DraftClawFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = DraftClawLogging.parseLabel(self.label)
