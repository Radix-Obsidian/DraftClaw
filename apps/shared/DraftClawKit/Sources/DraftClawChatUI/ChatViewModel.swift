...
private let chatUILogger = Logger(subsystem: "ai.draftclaw", category: "DraftClawChatUI")

@MainActor
@Observable
public final class DraftClawChatViewModel {
    public private(set) var messages: [DraftClawChatMessage] = []
    ...
    public var attachments: [DraftClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [DraftClawChatPendingToolCall] = []
    public private(set) var sessions: [DraftClawChatSessionEntry] = []
    private let transport: any DraftClawChatTransport
    ...
    private var pendingToolCallsById: [String: DraftClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any DraftClawChatTransport) {
    ...
    public var sessionChoices: [DraftClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [DraftClawChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [DraftClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: DraftClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [DraftClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: DraftClawChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [DraftClawChatMessage]) -> [DraftClawChatMessage] {
        var result: [DraftClawChatMessage] = []
    ...
    private static func dedupeKey(for message: DraftClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [DraftClawChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> DraftClawChatAttachmentPayload in
            DraftClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> DraftClawChatSessionEntry {
        DraftClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: DraftClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: DraftClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: DraftClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = DraftClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> DraftClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
