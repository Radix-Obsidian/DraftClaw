...
@MainActor
struct DraftClawChatComposer: View {
    @Bindable var viewModel: DraftClawChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(DraftClawChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(DraftClawChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \DraftClawPendingAttachment.id)
                { (att: DraftClawPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            DraftClawPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(DraftClawChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(DraftClawChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(DraftClawChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(DraftClawChatTheme.subtleCard)
        .clipShape(Capsule())
    }
