...
import Foundation
import Testing
@testable import DraftClawChatUI
...
private final class TestChatTransport: @unchecked Sendable, DraftClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [DraftClawChatHistoryPayload]
    private let sessionsResponses: [DraftClawChatSessionsListResponse]

    private let stream: AsyncStream<DraftClawChatTransportEvent>
    private let continuation: AsyncStream<DraftClawChatTransportEvent>.Continuation

    init(
        historyResponses: [DraftClawChatHistoryPayload],
        sessionsResponses: [DraftClawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<DraftClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> DraftClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? DraftClawChatHistoryPayload(
        ...
        attachments _: [DraftClawChatAttachmentPayload]) async throws -> DraftClawChatSendResponse
    {
        ...
        return DraftClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> DraftClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? DraftClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: DraftClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = DraftClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = DraftClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { DraftClawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = DraftClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = DraftClawChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = DraftClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { DraftClawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
