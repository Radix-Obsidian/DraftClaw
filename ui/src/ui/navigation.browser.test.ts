...
import { DraftClawApp } from "./app";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = DraftClawApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("draftclaw-app") as DraftClawApp;
...
beforeEach(() => {
  DraftClawApp.prototype.connect = () => {
...
afterEach(() => {
  DraftClawApp.prototype.connect = originalConnect;
  window.__OPENCLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/draftclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/draftclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/draftclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENCLAW_CONTROL_UI_BASE_PATH__ = "/draftclaw";
    const app = mountApp("/draftclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/draftclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/draftclaw/sessions");
  });
