...
service via the external `draftclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`OPENCLAW_PROFILE`; legacy `com.draftclaw.*` is supported).
...
- Writes `~/.draftclaw/disable-launchagent`.
...
`--attach-only` (or `--no-launchd`). This sets `~/.draftclaw/disable-launchagent`,
