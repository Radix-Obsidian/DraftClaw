---
summary: "DraftClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up DraftClaw on a Raspberry Pi
  - Running DraftClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# DraftClaw on Raspberry Pi

## Goal

Run a persistent, always-on DraftClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most DraftClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled draftclaw

# Enable if not
sudo systemctl enable draftclaw

# Start on boot
sudo systemctl start draftclaw
```
...
```bash
# Check logs
journalctl -u draftclaw --no-pager -n 100

# Common fix: rebuild
cd ~/draftclaw  # if using hackable install
npm run build
sudo systemctl restart draftclaw
```
