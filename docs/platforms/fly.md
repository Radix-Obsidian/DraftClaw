---
title: Fly.io
description: Deploy DraftClaw on Fly.io
---

# Fly.io Deployment

**Goal:** DraftClaw Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
fly apps create my-draftclaw

# Create a persistent volume (1GB is usually enough)
fly volumes create draftclaw_data --size 1 --region iad
```
...
```toml
app = "my-draftclaw"  # Your app name
primary_region = "iad"
...
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `draftclaw.json` where they could be accidentally exposed or logged.
...
**Note:** With `OPENCLAW_STATE_DIR=/data`, the config path is `/data/draftclaw.json`.
...
Or visit `https://my-draftclaw.fly.dev/`
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/draftclaw.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/draftclaw.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/draftclaw.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/draftclaw.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/draftclaw.json"
```
...
```bash
# List current IPs
fly ips list -a my-draftclaw

# Release public IPs
fly ips release <public-ipv4> -a my-draftclaw
fly ips release <public-ipv6> -a my-draftclaw
...
fly ips allocate-v6 --private -a my-draftclaw
```
...
fly proxy 3000:3000 -a my-draftclaw
...
```bash
fly ssh console -a my-draftclaw
```
