...
read_when:
  - Setting up Twitch chat integration for DraftClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. DraftClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
draftclaw plugins install @draftclaw/twitch
```

Local checkout (when running from a git repo):

```bash
draftclaw plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "draftclaw", // Bot's Twitch account
