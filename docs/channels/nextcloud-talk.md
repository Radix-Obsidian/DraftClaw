...
```bash
draftclaw plugins install @draftclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
draftclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "DraftClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `draftclaw pairing list nextcloud-talk`
  - `draftclaw pairing approve nextcloud-talk <CODE>`
