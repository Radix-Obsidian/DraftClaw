...
Tlon is a decentralized messenger built on Urbit. DraftClaw connects to your Urbit ship and can
...
```bash
draftclaw plugins install @draftclaw/tlon
```

Local checkout (when running from a git repo):

```bash
draftclaw plugins install ./extensions/tlon
```
...
Use these with `draftclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, DraftClaw replies in-thread.
