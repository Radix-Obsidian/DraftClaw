...
3. Set tokens for DraftClaw and start the gateway.
...
DraftClaw can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, DraftClaw falls back to the user token.
...
By default, DraftClaw replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, DraftClaw infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `draftclaw pairing approve slack <code>`.
...
- On startup, DraftClaw resolves channel/user names in allowlists to IDs (when tokens allow)
