...
Matrix is an open, decentralized messaging protocol. DraftClaw connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: `draftclaw plugins install @draftclaw/matrix`
   - From a local checkout: `draftclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: DraftClaw calls the same
     login endpoint, stores the access token in `~/.draftclaw/credentials/matrix/credentials.json`,
...
- On first connection, DraftClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  DraftClaw logs a warning.
...
`~/.draftclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, DraftClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
