...
- Jobs persist under `~/.draftclaw/cron/` so restarts don’t lose schedules.
...
draftclaw cron list
draftclaw cron run <job-id> --force
draftclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.draftclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `draftclaw cron add/edit` or the cron
...
- Job store: `~/.draftclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.draftclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.draftclaw/cron/jobs.json",
...
draftclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
draftclaw cron edit <jobId> --agent ops
draftclaw cron edit <jobId> --clear-agent
```
...
```bash
draftclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`draftclaw system event`](/cli/system).
