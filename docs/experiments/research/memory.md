...
read_when:
  - Designing workspace memory (~/.draftclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep DraftClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.draftclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.draftclaw/workspace` as canonical human-readable memory.
...
```
~/.draftclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in DraftClaw**, but keep a separable core library.

### Why integrate into DraftClaw?

- DraftClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `draftclaw memory recall "…" --k 25 --since 30d`
  - `draftclaw memory reflect --since 7d`
...
Pragmatic take for `~/.draftclaw/workspace`:
