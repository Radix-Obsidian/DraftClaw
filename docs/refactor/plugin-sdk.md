...
- Publish as `draftclaw/plugin-sdk` (or export from core under `draftclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `DraftClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: DraftClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: DraftClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: DraftClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: DraftClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: DraftClawConfig): string;
  };
};
```
...
- Introduce `draftclaw/plugin-sdk`.
- Add `api.runtime` to `DraftClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `draftclawRuntime: ">=2026.2.0"`).
