...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.draftclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.draftclaw/node.json` (node id + pairing token).
...
Path: `~/.draftclaw/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.draftclaw/exec-approvals.sock",
...
- Unix socket at `~/.draftclaw/exec-approvals.sock` (0600).
