...
- `/usage cost` in chats: local cost summary aggregated from DraftClaw session logs.
- CLI: `draftclaw status --usage` prints a full per-provider breakdown.
- CLI: `draftclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
