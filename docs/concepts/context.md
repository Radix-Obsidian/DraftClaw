...
read_when:
  - You want to understand what “context” means in DraftClaw
...
“Context” is **everything DraftClaw sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (DraftClaw-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How DraftClaw builds the system prompt

The system prompt is **DraftClaw-owned** and rebuilt each run. It includes:
...
By default, DraftClaw injects a fixed set of workspace files (if present):
