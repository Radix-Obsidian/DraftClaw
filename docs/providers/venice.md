---
summary: "Use Venice AI privacy-focused models in DraftClaw"
read_when:
  - You want privacy-focused inference in DraftClaw
...
```bash
draftclaw onboard --auth-choice venice-api-key
```
...
```bash
draftclaw chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, DraftClaw shows all available Venice models. Pick based on your needs:
...
```bash
draftclaw models set venice/claude-opus-45
draftclaw models set venice/llama-3.3-70b
```

List all available models:

```bash
draftclaw models list | grep venice
```

## Configure via `draftclaw configure`

1. Run `draftclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
DraftClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
draftclaw chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
draftclaw chat --model venice/claude-opus-45

# Use uncensored model
draftclaw chat --model venice/venice-uncensored

# Use vision model with image
draftclaw chat --model venice/qwen3-vl-235b-a22b

# Use coding model
draftclaw chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `draftclaw models list` to see currently available models. Some models may be temporarily offline.
