---
summary: "Use MiniMax M2.1 in DraftClaw"
read_when:
  - You want MiniMax models in DraftClaw
...
```bash
draftclaw plugins enable minimax-portal-auth  # skip if already loaded.
draftclaw gateway restart  # restart if gateway is already running
draftclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/draftclaw/draftclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `draftclaw.json`:
...
- Use `draftclaw models list` and `draftclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `draftclaw configure` and selecting **MiniMax M2.1**, or
