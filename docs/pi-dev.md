...
This guide summarizes a sane workflow for working on the pi integration in DraftClaw.
...
- Trigger the agent directly:
  - `pnpm draftclaw agent --message "Hello" --thinking low`
...
State lives under the DraftClaw state directory. Default is `~/.draftclaw`. If `OPENCLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `draftclaw.json` for config
...
- https://docs.draftclaw.ai/testing
- https://docs.draftclaw.ai/start/getting-started
