...
### `docs.draftclaw.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.draftclaw.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.draftclaw.ai` to the allowlist, then retry.
...
- Check the allowlist: `draftclaw config get agents.defaults.models`
...
If you can, include the relevant log tail from `draftclaw logs --follow`.
