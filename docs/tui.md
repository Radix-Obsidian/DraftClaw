...
```bash
draftclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `draftclaw tui --deliver`
...
- Check the Gateway logs: `draftclaw logs --follow`.
- Confirm the agent can run: `draftclaw status` and `draftclaw models status`.
...
- No agents in picker: check `draftclaw agents list` and your routing config.
