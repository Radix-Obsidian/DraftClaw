...
DraftClaw features that can generate provider usage or paid API calls.
...
- `draftclaw status --usage` and `draftclaw channels list` show provider **usage windows**
...
DraftClaw can pick up credentials from:
...
- `draftclaw status --usage`
- `draftclaw models status --json`
...
`draftclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
