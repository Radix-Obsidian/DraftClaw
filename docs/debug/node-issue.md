...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in DraftClaw
...
Running DraftClaw via Node with `tsx` fails at startup with:

```
[draftclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `draftclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch draftclaw.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node draftclaw.mjs status` works on Node 25.
