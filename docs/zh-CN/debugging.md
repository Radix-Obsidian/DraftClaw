...
当你需要切换一些不常用的设置而又不想编辑 `draftclaw.json` 时，这非常方便。
...
- **全局 `--dev`（配置文件）：** 将状态隔离到 `~/.draftclaw-dev`，并将 Gateway网关默认端口设为 `19001`（派生端口随之偏移）。
...
如果你还没有全局安装，可通过 `pnpm draftclaw ...` 运行 CLI。
...
DraftClaw 可以在任何过滤/格式化之前记录**原始助手流**。
