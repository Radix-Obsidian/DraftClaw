...
### `docs.draftclaw.ai` 显示 SSL 错误（Comcast/Xfinity）

某些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 屏蔽 `docs.draftclaw.ai`。
禁用 Advanced Security 或将 `docs.draftclaw.ai` 添加到允许列表，然后重试。
...
- 检查允许列表：`draftclaw config get agents.defaults.models`
...
如果可以，请附上 `draftclaw logs --follow` 中相关的日志尾部内容。
