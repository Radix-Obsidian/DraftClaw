...
- 替代旧版 `gateway` 命令。CLI 入口：`draftclaw gateway`。
...
- 配置热重载会监视 `~/.draftclaw/draftclaw.json`（或 `OPENCLAW_CONFIG_PATH`）。
...
- 默认在 `canvasHost.port`（默认 `18793`）启动 Canvas 文件服务器，从 `~/.draftclaw/workspace/canvas` 提供 `http://<gateway-host>:18793/__draftclaw__/canvas/` 服务。通过 `canvasHost.enabled=false` 或 `OPENCLAW_SKIP_CANVAS_HOST=1` 禁用。
...
- macOS：`bot.molt.<profile>`（旧版 `com.draftclaw.*` 可能仍然存在）
- Linux：`draftclaw-gateway-<profile>.service`
- Windows：`DraftClaw Gateway网关 (<profile>)`
...
- 在 `--dev` 下运行 `setup`/`onboard` 时，`agents.defaults.workspace` 默认变为 `~/.draftclaw/workspace-dev`。
...
- `health` — 完整健康快照（与 `draftclaw health --json` 形状相同）。
- `status` — 简短摘要。
...
- LaunchAgents 是按用户的，需要已登录的会话；对于无头设置，请使用自定义 LaunchDaemon（未随附）。
  - `draftclaw gateway install` 写入 `~/Library/LaunchAgents/bot.molt.gateway.plist`
    （或 `bot.molt.<profile>.plist`；旧版 `com.draftclaw.*` 会被清理）。
  - `draftclaw doctor` 审计 LaunchAgent 配置，并可将其更新为当前推荐的默认值。
...
- 如果检测到其他类似 Gateway网关的服务，CLI 会发出警告，除非它们是 DraftClaw 配置文件服务。
  我们仍建议大多数场景下**每台机器一个 Gateway网关**；使用隔离的配置文件/端口实现冗余或救援机器人。参见 [多 Gateway网关](/gateway/multiple-gateways)。
  - 清理：`draftclaw gateway uninstall`（当前服务）和 `draftclaw doctor`（旧版迁移）。
- `gateway install` 在已安装时为空操作；使用 `draftclaw gateway install --force` 重新安装（配置文件/环境/路径变更）。

捆绑的 Mac 应用：

- DraftClaw.app 可以捆绑一个基于 Node 的 Gateway网关中继，并安装按用户的 LaunchAgent，标签为
  `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.draftclaw.*` 标签仍可正常卸载）。
- 要正常停止，使用 `draftclaw gateway stop`（或 `launchctl bootout gui/$UID/bot.molt.gateway`）。
- 要重启，使用 `draftclaw gateway restart`（或 `launchctl kickstart -k gui/$UID/bot.molt.gateway`）。
  - `launchctl` 仅在 LaunchAgent 已安装时有效；否则先使用 `draftclaw gateway install`。
  - 运行命名配置文件时，将标签替换为 `bot.molt.<profile>`。

## 进程监管（systemd 用户单元）

DraftClaw 在 Linux/WSL2 上默认安装 **systemd 用户服务**。我们推荐单用户机器使用用户服务（更简单的环境，按用户配置）。对于多用户或始终在线的服务器，使用**系统服务**（无需 lingering，共享监管）。

`draftclaw gateway install` 写入用户单元。`draftclaw doctor` 审计该单元，并可将其更新为当前推荐的默认值。

创建 `~/.config/systemd/user/draftclaw-gateway[-<profile>].service`：

```
[Unit]
Description=DraftClaw Gateway网关 (profile: <profile>, v<version>)
...
**替代方案（系统服务）** - 对于始终在线或多用户服务器，可以安装 systemd **系统**单元而非用户单元（无需 lingering）。创建 `/etc/systemd/system/draftclaw-gateway[-<profile>].service`（复制上述单元，将 `WantedBy=multi-user.target`，设置 `User=` + `WorkingDirectory=`），然后：
...
- `draftclaw gateway health|status` — 通过 Gateway网关 WS 请求健康/状态信息。
- `draftclaw message send --target <num> --message "hi" [--media ...]` — 通过 Gateway网关发送（对 WhatsApp 具有幂等性）。
- `draftclaw agent --message "hi" --to <num>` — 运行智能体回合（默认等待最终结果）。
- `draftclaw gateway call <method> --params '{"k":"v"}'` — 用于调试的原始方法调用器。
- `draftclaw gateway stop|restart` — 停止/重启受监管的 Gateway网关服务（launchd/systemd）。
- Gateway网关辅助子命令假设 Gateway网关已在 `--url` 上运行；它们不再自动启动 Gateway网关。

## 迁移指南

- 弃用 `draftclaw gateway` 和旧版 TCP 控制端口的用法。
