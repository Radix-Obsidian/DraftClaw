...
- 使用 macOS 应用的**通过 SSH 远程连接**模式（设置 → 通用 → "DraftClaw 运行位置"）。
...
- `draftclaw health` 和 `draftclaw status --deep` 现在通过 `ws://127.0.0.1:18789` 访问远程 Gateway网关。
- `draftclaw gateway {status,health,send,agent,call}` 也可以在需要时通过 `--url` 指向转发的 URL。
