...
**目标：** 在 [Fly.io](https://fly.io) 机器上运行 DraftClaw Gateway网关，配备持久化存储、自动 HTTPS 和 Discord/渠道访问。
...
- 请像对待密码一样对待这些令牌。
- **所有 API 密钥和令牌优先使用环境变量而非配置文件**。这样可以避免密钥出现在 `draftclaw.json` 中，防止意外暴露或被记录到日志。
...
**注意：** 当 `OPENCLAW_STATE_DIR=/data` 时，配置路径为 `/data/draftclaw.json`。
...
或访问 `https://my-draftclaw.fly.dev/`
...
如果使用 `--allow-unconfigured`，Gateway网关会创建一个最小配置。你在 `/data/draftclaw.json` 的自定义配置应在重启后被读取。
