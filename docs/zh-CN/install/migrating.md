---
read_when:
  - 你正在将 DraftClaw 迁移到新的笔记本/服务器
  - 你想保留会话、认证和渠道登录状态（WhatsApp 等）
summary: 将 DraftClaw 安装从一台机器迁移到另一台
title: 迁移指南
x-i18n:
...
本指南介绍如何将 DraftClaw Gateway网关从一台机器迁移到另一台，**无需重新进行新手引导**。

迁移在概念上很简单：

- 复制**状态目录**（`$OPENCLAW_STATE_DIR`，默认：`~/.draftclaw/`）— 包含配置、认证、会话和渠道状态。
- 复制你的**工作区**（默认 `~/.draftclaw/workspace/`）— 包含你的智能体文件（记忆、提示词等）。
...
- `--profile <name>`（通常变为 `~/.draftclaw-<profile>/`）
...
如果你有多个配置文件/状态目录（例如 `~/.draftclaw-main`、`~/.draftclaw-work`），请分别归档。

### 步骤 1 — 在新机器上安装 DraftClaw

在**新**机器上安装 CLI（如有需要也安装 Node）：

- 参见：[安装](/install)

在此阶段，新手引导创建一个全新的 `~/.draftclaw/` 是没问题的 — 你将在下一步覆盖它。
...
### 坑：只复制了 `draftclaw.json`

`draftclaw.json` 是不够的。许多提供商将状态存储在：
...
- `draftclaw status` 显示 Gateway网关正在运行
...
- [DraftClaw 将数据存储在哪里？](/help/faq#where-does-draftclaw-store-its-data)
