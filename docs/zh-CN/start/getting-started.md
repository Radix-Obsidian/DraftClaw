...
- **推荐：** Brave Search API 密钥用于网络搜索。最简单的方式：
  `draftclaw configure --section web` （存储 `tools.web.search.apiKey`）。
  参见 [网络工具](/tools/web)。
...
```bash
curl -fsSL https://draftclaw.bot/install.sh | bash
```
...
- OAuth 凭据（旧版导入）： `~/.draftclaw/credentials/oauth.json`
...
如果你尚未进行全局安装，请通过以下方式运行新手引导流程 `pnpm draftclaw ...` （从仓库中）。
...
如果 `draftclaw health` 显示"未配置认证"，请返回向导设置 OAuth/密钥认证——智能体在没有认证的情况下将无法响应。

提示： `draftclaw status --all` 是最佳的可粘贴只读调试报告。
健康探针： `draftclaw health` （或 `draftclaw status --deep`）向运行中的 Gateway网关请求健康快照。
