...
DraftClaw 拥有三个 Vitest 测试套件（单元/集成、端到端、实时）和一小组 Docker 运行器。
...
### 第二层：Gateway网关 + 开发智能体冒烟测试（"@draftclaw"实际做什么）
...
- Gemini API 与 Gemini CLI：
  - API：DraftClaw 通过 HTTP 调用 Google 的托管 Gemini API（API 密钥/profile 认证）；这是大多数用户所说的"Gemini"。
  - CLI：DraftClaw 调用本地 `gemini` 二进制文件；它有自己的认证，行为可能不同（流式传输/工具支持/版本差异）。
...
- OpenRouter：`openrouter/...`（数百个模型；使用 `draftclaw models scan` 查找支持工具+图像的候选模型）
...
- 如果实时测试提示"无凭证"，按照调试 `draftclaw models list`/模型选择的方式来调试。

- Profile 存储：`~/.draftclaw/credentials/`（首选；测试中"profile 密钥"的含义）
- 配置：`~/.draftclaw/draftclaw.json`（或 `OPENCLAW_CONFIG_PATH`）
...
- `OPENCLAW_CONFIG_DIR=...`（默认：`~/.draftclaw`）挂载到 `/home/node/.draftclaw`
- `OPENCLAW_WORKSPACE_DIR=...`（默认：`~/.draftclaw/workspace`）挂载到 `/home/node/.draftclaw/workspace`
