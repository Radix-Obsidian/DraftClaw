...
你可以在 `~/.draftclaw/draftclaw.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/draftclaw.log"
  }
}
```
...
```bash
draftclaw channels logs --channel whatsapp
```
...
所有日志配置位于 `~/.draftclaw/draftclaw.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/draftclaw/draftclaw-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- DraftClaw 目前通过 **OTLP/HTTP (protobuf)** 导出。
...
- 你也可以使用 `draftclaw plugins enable diagnostics-otel` 来启用插件。
...
- `draftclaw.tokens`（计数器，属性：`draftclaw.token`、`draftclaw.channel`、`draftclaw.provider`、`draftclaw.model`）
- `draftclaw.cost.usd`（计数器，属性：`draftclaw.channel`、`draftclaw.provider`、`draftclaw.model`）
- `draftclaw.run.duration_ms`（直方图，属性：`draftclaw.channel`、`draftclaw.provider`、`draftclaw.model`）
- `draftclaw.context.tokens`（直方图，属性：`draftclaw.context`、`draftclaw.channel`、`draftclaw.provider`、`draftclaw.model`）

消息流：

- `draftclaw.webhook.received`（计数器，属性：`draftclaw.channel`、`draftclaw.webhook`）
- `draftclaw.webhook.error`（计数器，属性：`draftclaw.channel`、`draftclaw.webhook`）
- `draftclaw.webhook.duration_ms`（直方图，属性：`draftclaw.channel`、`draftclaw.webhook`）
- `draftclaw.message.queued`（计数器，属性：`draftclaw.channel`、`draftclaw.source`）
- `draftclaw.message.processed`（计数器，属性：`draftclaw.channel`、`draftclaw.outcome`）
- `draftclaw.message.duration_ms`（直方图，属性：`draftclaw.channel`、`draftclaw.outcome`）

队列 + 会话：

- `draftclaw.queue.lane.enqueue`（计数器，属性：`draftclaw.lane`）
- `draftclaw.queue.lane.dequeue`（计数器，属性：`draftclaw.lane`）
- `draftclaw.queue.depth`（直方图，属性：`draftclaw.lane` 或 `draftclaw.channel=heartbeat`）
- `draftclaw.queue.wait_ms`（直方图，属性：`draftclaw.lane`）
- `draftclaw.session.state`（计数器，属性：`draftclaw.state`、`draftclaw.reason`）
- `draftclaw.session.stuck`（计数器，属性：`draftclaw.state`）
- `draftclaw.session.stuck_age_ms`（直方图，属性：`draftclaw.state`）
- `draftclaw.run.attempt`（计数器，属性：`draftclaw.attempt`）

### 导出的 span（名称 + 关键属性）

- `draftclaw.model.usage`
  - `draftclaw.channel`、`draftclaw.provider`、`draftclaw.model`
  - `draftclaw.sessionKey`、`draftclaw.sessionId`
  - `draftclaw.tokens.*`（input/output/cache_read/cache_write/total）
- `draftclaw.webhook.processed`
  - `draftclaw.channel`、`draftclaw.webhook`、`draftclaw.chatId`
- `draftclaw.webhook.error`
  - `draftclaw.channel`、`draftclaw.webhook`、`draftclaw.chatId`、`draftclaw.error`
- `draftclaw.message.processed`
  - `draftclaw.channel`、`draftclaw.outcome`、`draftclaw.chatId`、`draftclaw.messageId`、`draftclaw.sessionKey`、`draftclaw.sessionId`、`draftclaw.reason`
- `draftclaw.session.stuck`
  - `draftclaw.state`、`draftclaw.ageMs`、`draftclaw.queueDepth`、`draftclaw.sessionKey`、`draftclaw.sessionId`
...
- **Gateway网关不可达？** 先运行 `draftclaw doctor`。
