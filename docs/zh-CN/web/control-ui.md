...
- 可选前缀：设置 `gateway.controlUi.basePath`（例如 `/draftclaw`）
...
如果页面无法加载，请先启动 Gateway网关：`draftclaw gateway`。
...
审批后，设备会被记住，除非你使用 `draftclaw devices revoke --device <id> --role <role>` 撤销，否则不需要重新审批。参见
...
- 配置：查看/编辑 `~/.draftclaw/draftclaw.json`（`config.get`、`config.set`）
...
（`tailscale-user-login`）进行认证。DraftClaw
