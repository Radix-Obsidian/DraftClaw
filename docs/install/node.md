...
read_when:
  - "You installed DraftClaw but `draftclaw` is “command not found”"
...
DraftClaw’s runtime baseline is **Node 22+**.

If you can run `npm install -g draftclaw@latest` but later see `draftclaw: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
