...
- Replaces the legacy `gateway` command. CLI entry point: `draftclaw gateway`.
...
draftclaw gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.draftclaw/draftclaw.json` (or `OPENCLAW_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__draftclaw__/canvas/` from `~/.draftclaw/workspace/canvas`. Disable with `canvasHost.enabled=false` or `OPENCLAW_SKIP_CANVAS_HOST=1`.
...
- macOS: `bot.molt.<profile>` (legacy `com.draftclaw.*` may still exist)
- Linux: `draftclaw-gateway-<profile>.service`
- Windows: `DraftClaw Gateway (<profile>)`

Install metadata is embedded in the service config:

- `OPENCLAW_SERVICE_MARKER=draftclaw`
...
```bash
draftclaw --dev setup
draftclaw --dev gateway --allow-unconfigured
# then target the dev instance:
draftclaw --dev status
draftclaw --dev health
```

Defaults (can be overridden via env/flags/config):

- `OPENCLAW_STATE_DIR=~/.draftclaw-dev`
- `OPENCLAW_CONFIG_PATH=~/.draftclaw-dev/draftclaw.json`
- `OPENCLAW_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.draftclaw/workspace-dev` when you run `setup`/`onboard` under `--dev`.
...
```bash
draftclaw --profile main gateway install
draftclaw --profile rescue gateway install
```

Example:

```bash
OPENCLAW_CONFIG_PATH=~/.draftclaw/a.json OPENCLAW_STATE_DIR=~/.draftclaw-a draftclaw gateway --port 19001
OPENCLAW_CONFIG_PATH=~/.draftclaw/b.json OPENCLAW_STATE_DIR=~/.draftclaw-b draftclaw gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `draftclaw health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `draftclaw`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `draftclaw gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.draftclaw.*` is cleaned up).
  - `draftclaw doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are DraftClaw profile services.
  ...
  - Cleanup: `draftclaw gateway uninstall` (current service) and `draftclaw doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `draftclaw gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- DraftClaw.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.draftclaw.*` labels still unload cleanly).
- To stop it cleanly, use `draftclaw gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `draftclaw gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `draftclaw gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

DraftClaw installs a **systemd user service** by default on Linux/WSL2. We
...
`draftclaw gateway install` writes the user unit. `draftclaw doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/draftclaw-gateway[-<profile>].service`:

```
[Unit]
Description=DraftClaw Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/draftclaw gateway --port 18789
Restart=always
RestartSec=5
...
```
systemctl --user enable --now draftclaw-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/draftclaw-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now draftclaw-gateway[-<profile>].service
```
...
- `draftclaw gateway health|status` — request health/status over the Gateway WS.
- `draftclaw message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `draftclaw agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `draftclaw gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `draftclaw gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `draftclaw gateway` and the legacy TCP control port.
