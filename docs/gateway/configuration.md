---
summary: "All configuration options for ~/.draftclaw/draftclaw.json with examples"
read_when:
...
DraftClaw reads an optional **JSON5** config from `~/.draftclaw/draftclaw.json` (comments + trailing commas allowed).

If the file is missing, DraftClaw uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.draftclaw/workspace`). You usually only need a config to:
...
DraftClaw only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `draftclaw doctor`, `draftclaw logs`, `draftclaw health`, `draftclaw status`, `draftclaw service`, `draftclaw help`).
- Run `draftclaw doctor` to see the exact issues.
- Run `draftclaw doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `draftclaw config set`. Keep a backup of `~/.draftclaw/draftclaw.json`.
...
```bash
draftclaw gateway call config.get --params '{}' # capture payload.hash
draftclaw gateway call config.apply --params '{
...
```bash
draftclaw gateway call config.get --params '{}' # capture payload.hash
draftclaw gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.draftclaw/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@draftclaw", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.draftclaw/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.draftclaw/workspace" }],
}
```
...
{ "$include": "/etc/draftclaw/base.json5" } // absolute
...
```json5
// ~/.draftclaw/clients/mueller/agents.json5
[
...
```json5
// ~/.draftclaw/clients/mueller/broadcast.json5
{
...
DraftClaw reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, DraftClaw runs your login shell and imports only the missing expected keys (never overrides).
...
DraftClaw stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.draftclaw/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.draftclaw/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`)
...
- `~/.draftclaw/agent/*` (migrated by `draftclaw doctor` into `~/.draftclaw/agents/<defaultAgentId>/agent/*`)
...
On first use, DraftClaw imports `oauth.json` entries into `auth-profiles.json`.
...
If set, DraftClaw derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/draftclaw/draftclaw-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/draftclaw/draftclaw.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/draftclaw/draftclaw.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.draftclaw/credentials/whatsapp/biz
          // authDir: "~/.draftclaw/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `draftclaw doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@draftclaw", "draftclaw"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@draftclaw"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.draftclaw/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.draftclaw/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.draftclaw/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.draftclaw/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.draftclaw/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.draftclaw/workspace-home" },
      { id: "work", workspace: "~/.draftclaw/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `draftclaw.json`).
...
DraftClaw starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["draftclaw-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-draftclaw",
...
DraftClaw starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `draftclaw plugins install @draftclaw/mattermost` (or `./extensions/mattermost` from a git checkout).
...
DraftClaw starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
DraftClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, DraftClaw
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/draftclaw" } },
}
```
...
When a file exceeds this limit, DraftClaw logs a warning and injects a truncated
...
message envelopes). If unset, DraftClaw uses the host timezone at runtime.
...
`[draftclaw]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[draftclaw]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[draftclaw]"`, DraftClaw will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, DraftClaw generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.draftclaw/settings/tts.json",
      elevenlabs: {
...
DraftClaw also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, DraftClaw currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `draftclaw configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:draftclaw`: all built-in DraftClaw tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.draftclaw/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.draftclaw/sandboxes",
        docker: {
          image: "draftclaw-sandbox:bookworm-slim",
          containerPrefix: "draftclaw-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "draftclaw-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "draftclaw-sandbox-browser:bookworm-slim",
          containerPrefix: "draftclaw-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
DraftClaw uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.draftclaw/agents/<agentId>/agent/models.json` or by defining the same schema inside your
DraftClaw config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, DraftClaw writes/merges a `models.json` into
`~/.draftclaw/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. DraftClaw uses
...
- Shortcut: `draftclaw onboard --auth-choice opencode-zen`.
...
Shortcut: `draftclaw onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `draftclaw onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `draftclaw onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `draftclaw onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `draftclaw onboard --auth-choice minimax-api`.
...
- Override the agent config root with `OPENCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.draftclaw/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.draftclaw/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.draftclaw/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, DraftClaw stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.draftclaw/skills` (workspace skills
...
from `~/.draftclaw/extensions`, `<workspace>/.draftclaw/extensions`, plus any
...
### `browser` (draftclaw-managed browser)

DraftClaw can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for draftclaw and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (DraftClaw.app menubar, or `draftclaw gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      draftclaw: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/draftclaw" }
...
- Examples: `"/ui"`, `"/draftclaw"`, `"/apps/draftclaw"`.
...
- When a connection comes from one of these IPs, DraftClaw uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `draftclaw gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
...
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  ...
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. DraftClaw
...
- DraftClaw.app watches `~/.draftclaw/draftclaw.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.draftclaw/draftclaw.json` (or `OPENCLAW_CONFIG_PATH`) and applies changes automatically.
...
- `~/.draftclaw/draftclaw.json` (or `OPENCLAW_CONFIG_PATH`)
...
- `draftclaw --dev …` → uses `~/.draftclaw-dev` + shifts ports from base `19001`
- `draftclaw --profile <name> …` → uses `~/.draftclaw-<name>` (port via config/env/flags)
...
Gmail helper config (used by `draftclaw webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "draftclaw@gmail.com",
...
Note: when `tailscale.mode` is on, DraftClaw defaults `serve.path` to `/` so
...
Default root: `~/.draftclaw/workspace/canvas`  
Default port: `18793` (chosen to avoid the draftclaw browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__draftclaw__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__draftclaw__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.draftclaw/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.draftclaw/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.draftclaw/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.draftclaw/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_draftclaw-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `draftclaw` (advertises `draftclaw.local`). Override with `OPENCLAW_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_draftclaw-gw._tcp` under `~/.draftclaw/dns/` using the configured discovery domain (example: `draftclaw.internal.`).
