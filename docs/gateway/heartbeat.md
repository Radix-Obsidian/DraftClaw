...
- During heartbeat runs, DraftClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `draftclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, DraftClaw skips the heartbeat run entirely (no model call).
...
```bash
draftclaw system event --text "Check for urgent follow-ups" --mode now
```
