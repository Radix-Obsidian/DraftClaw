---
summary: "Quick troubleshooting guide for common DraftClaw failures"
read_when:
...
When DraftClaw misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `draftclaw status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `draftclaw status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `draftclaw status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `draftclaw gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `draftclaw channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `draftclaw gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `draftclaw logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `draftclaw status --all` (it redacts tokens). If you paste `draftclaw status`, consider setting `OPENCLAW_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  draftclaw models auth setup-token --provider anthropic
  ```
...
```bash
draftclaw models auth paste-token --provider anthropic
draftclaw models status
```
...
- Preferred: `draftclaw logs --follow`
- File logs (always): `/tmp/draftclaw/draftclaw-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$OPENCLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u draftclaw-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  draftclaw config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  draftclaw config set gateway.mode remote
  draftclaw config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `draftclaw setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.draftclaw/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `draftclaw doctor`
...
- then run `draftclaw sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `draftclaw gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `draftclaw gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `draftclaw gateway status` and `draftclaw doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
...
**If `draftclaw gateway status` shows a config mismatch**
...
- Fix: rerun `draftclaw gateway install --force` from the same `--profile` / `OPENCLAW_STATE_DIR` you want the service to use.

**If `draftclaw gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `draftclaw doctor` to update it (or `draftclaw gateway install --force` for a full rewrite).
...
**If `draftclaw gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/draftclaw` on disk.
...
Symptoms: `pwd` or file tools show `~/.draftclaw/sandboxes/...` even though you
...
DraftClaw intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `draftclaw models list` or
  `draftclaw models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${OPENCLAW_CONFIG_PATH:-$HOME/.draftclaw/draftclaw.json}"
```
...
tail -f "$(ls -t /tmp/draftclaw/draftclaw-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
draftclaw pairing list <channel>
```
...
```bash
draftclaw logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.draftclaw/agents/<agentId>/sessions/
```
...
draftclaw logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
draftclaw gateway --verbose
```

If you’re logged out / unlinked:

```bash
draftclaw channels logout
trash "${OPENCLAW_STATE_DIR:-$HOME/.draftclaw}/credentials" # if logout can't cleanly remove everything
draftclaw channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/draftclaw/draftclaw-*.log | head -1)" | tail -20
```

### High Memory Usage

DraftClaw keeps conversation history in memory.
...
DraftClaw now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
```bash
draftclaw doctor
draftclaw doctor --fix
```

Notes:

- `draftclaw doctor` reports every invalid entry.
- `draftclaw doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `draftclaw logs`, `draftclaw health`, `draftclaw status`, `draftclaw gateway status`, and `draftclaw gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/draftclaw/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `draftclaw models status` (CLI).
...
```bash
curl -fsSL https://draftclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
```
...
5. Run `draftclaw channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. DraftClaw scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/draftclaw/draftclaw/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.draftclaw.* still works)
```
...
Ensure the global `draftclaw` CLI is installed and matches the app version:

```bash
draftclaw --version
npm install -g draftclaw@<version>
```
...
#   ${OPENCLAW_CONFIG_PATH:-$HOME/.draftclaw/draftclaw.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
draftclaw gateway --verbose
draftclaw channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/draftclaw/draftclaw-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$OPENCLAW_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.draftclaw/logs/...`; profiles use `~/.draftclaw-<profile>/logs/...`)<br />Linux: `journalctl --user -u draftclaw-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "DraftClaw Gateway (<profile>)" /V /FO LIST` |
...
draftclaw gateway status --deep

# Is the gateway reachable?
draftclaw health --json
# If it fails, rerun with connection details:
draftclaw health --verbose
...
tail -20 /tmp/draftclaw/draftclaw-*.log
```
...
# draftclaw gateway uninstall

trash "${OPENCLAW_STATE_DIR:-$HOME/.draftclaw}"
draftclaw channels login         # re-pair WhatsApp
draftclaw gateway restart           # or: draftclaw gateway
```
...
1. Check logs first: `/tmp/draftclaw/` (default: `draftclaw-YYYY-MM-DD.log`, or your configured `logging.file`)
