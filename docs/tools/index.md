---
summary: "Agent tool surface for DraftClaw (browser, canvas, nodes, message, cron) replacing legacy `draftclaw-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `draftclaw-*` skills
title: "Tools"
---

# Tools (DraftClaw)

DraftClaw exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `draftclaw-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `draftclaw.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, DraftClaw logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`draftclaw node run`).
...
- Requires a Brave API key (recommended: `draftclaw configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated DraftClaw-managed browser.
...
- Quick smoke: `draftclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `draftclaw gateway` restart in-place)
