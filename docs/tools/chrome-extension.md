---
summary: "Chrome extension: let DraftClaw drive your existing Chrome tab"
read_when:
...
The DraftClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate draftclaw-managed Chrome profile.
...
DraftClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the DraftClaw release (npm package) as static files. There is no separate “build” step.

After upgrading DraftClaw:

- Re-run `draftclaw browser extension install` to refresh the installed files under your DraftClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

DraftClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want DraftClaw to control.
...
- `ON`: attached; DraftClaw can drive that tab.
...
Debugging: `draftclaw sandbox explain`
...
`draftclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `draftclaw browser extension install` first to copy the extension to a stable location under your DraftClaw state directory.
...
- **This is not isolated** like the dedicated draftclaw-managed profile.
