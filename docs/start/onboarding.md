---
summary: "First-run onboarding flow for DraftClaw (macOS app)"
read_when:
...
- Writes credentials to `~/.draftclaw/credentials/oauth.json`
...
The app can install the global `draftclaw` CLI via npm/pnpm so terminal
...
On the first agent run, DraftClaw bootstraps a workspace (default `~/.draftclaw/workspace`):
...
```bash
draftclaw webhooks gmail setup --account you@gmail.com
```
...
- `~/.draftclaw/credentials/oauth.json`
