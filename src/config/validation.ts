...
import type { DraftClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: DraftClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: DraftClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = DraftClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as DraftClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as DraftClawConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as DraftClawConfig)),
    ),
  };
}
