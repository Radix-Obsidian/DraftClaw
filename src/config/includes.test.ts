...
const ETC_DRAFTCLAW_DIR = path.join(ROOT_DIR, "etc", "draftclaw");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "draftclaw.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcDraftClawPath(...parts: string[]) {
  return path.join(ETC_DRAFTCLAW_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcDraftClawPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "draftclaw.json"))).toEqual({
      shared: true,
    });
  });
});
