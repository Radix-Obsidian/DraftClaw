...
import { readConfigFileSnapshot, type DraftClawConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: DraftClawConfig, model: string): DraftClawConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("draftclaw plugins install")}\`.`,
    );
  }
