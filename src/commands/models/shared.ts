...
export async function updateConfig(
  mutator: (cfg: DraftClawConfig) => DraftClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: DraftClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: DraftClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("draftclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
