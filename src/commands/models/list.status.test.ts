...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveDraftClawAgentDir: vi.fn().mockReturnValue("/tmp/draftclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/draftclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/draftclaw-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveDraftClawAgentDir: mocks.resolveDraftClawAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveDraftClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/draftclaw-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/draftclaw-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/draftclaw-agent-custom");
