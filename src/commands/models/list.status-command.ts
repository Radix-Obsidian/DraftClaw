...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveDraftClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("draftclaw models auth setup-token")}\` or \`${formatCliCommand("draftclaw configure")}\`.`
          : `Run \`${formatCliCommand("draftclaw configure")}\` or set an API key env var.`;
