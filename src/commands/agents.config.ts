...
export function listAgentEntries(cfg: DraftClawConfig): AgentEntry[] {
...
function resolveAgentName(cfg: DraftClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: DraftClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: DraftClawConfig): AgentSummary[] {
