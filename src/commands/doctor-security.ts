...
import type { DraftClawConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: DraftClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("draftclaw security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("draftclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("draftclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("draftclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("draftclaw config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
