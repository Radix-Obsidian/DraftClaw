...
beforeAll(() => {
  previousProfile = process.env.DRAFTCLAW_PROFILE;
  process.env.DRAFTCLAW_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.DRAFTCLAW_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/draftclaw-root.js", () => ({
  resolveDraftClawPackageRoot: vi.fn().mockResolvedValue("/tmp/draftclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/draftclaw/pnpm-lock.yaml",
      markerPath: "/tmp/draftclaw/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("DraftClaw status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("draftclaw status --all") ||
          l.includes("draftclaw --profile isolated status --all") ||
          l.includes("draftclaw status --all") ||
          l.includes("draftclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.DRAFTCLAW_GATEWAY_TOKEN = "abcd1234";
    try {
