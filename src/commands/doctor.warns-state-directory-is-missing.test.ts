...
beforeEach(() => {
  ...
  resolveDraftClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.DRAFTCLAW_STATE_DIR;
  originalUpdateInProgress = process.env.DRAFTCLAW_UPDATE_IN_PROGRESS;
  process.env.DRAFTCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "draftclaw-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.DRAFTCLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.DRAFTCLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.DRAFTCLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveDraftClawPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadDraftClawPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/draftclaw.json",
    createConfigIO,
...
vi.mock("../infra/draftclaw-root.js", () => ({
  resolveDraftClawPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "draftclaw-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.DRAFTCLAW_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when DRAFTCLAW_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/draftclaw.json",
      exists: true,
      raw: "{}",
    ...
    process.env.DRAFTCLAW_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
