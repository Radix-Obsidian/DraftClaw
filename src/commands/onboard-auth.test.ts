...
const requireAgentDir = () => {
  const agentDir = process.env.DRAFTCLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("DRAFTCLAW_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.DRAFTCLAW_STATE_DIR;
  const previousAgentDir = process.env.DRAFTCLAW_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.DRAFTCLAW_AGENT_DIR;
    } else {
      process.env.DRAFTCLAW_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.DRAFTCLAW_OAUTH_DIR;
  });

  it("writes auth-profiles.json under DRAFTCLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "draftclaw-oauth-"));
    process.env.DRAFTCLAW_STATE_DIR = tempStateDir;
    process.env.DRAFTCLAW_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.DRAFTCLAW_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to DRAFTCLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "draftclaw-minimax-"));
    process.env.DRAFTCLAW_STATE_DIR = tempStateDir;
    process.env.DRAFTCLAW_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.DRAFTCLAW_AGENT_DIR;
