...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "DRAFTCLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("DRAFTCLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv DRAFTCLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("DRAFTCLAW_GATEWAY_PASSWORD");
  });
