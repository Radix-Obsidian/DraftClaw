...
function resolveMode(cfg: DraftClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("DraftClaw doctor");

  const root = await resolveDraftClawPackageRoot({
  ...
  let cfg: DraftClawConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("draftclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("draftclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("draftclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("draftclaw doctor --fix")}" to apply changes.`);
  }
