...
const SEARCH_TOOL = "https://docs.draftclaw.ai/mcp.SearchDraftClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.draftclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('draftclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.draftclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('draftclaw docs "your query"')}`);
    }
    return;
  }
