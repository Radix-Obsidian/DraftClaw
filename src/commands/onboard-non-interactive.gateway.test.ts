...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.DRAFTCLAW_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.DRAFTCLAW_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "draftclaw-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.DRAFTCLAW_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "draftclaw");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.DRAFTCLAW_STATE_DIR = stateDir;
    process.env.DRAFTCLAW_CONFIG_PATH = path.join(stateDir, "draftclaw.json");
