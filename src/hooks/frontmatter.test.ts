...
`;
    ...
    expect(parsed.draftclaw.emoji).toBe("💾");
    expect(parsed.draftclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.draftclaw.emoji).toBe("📝");
    expect(parsed.draftclaw.events).toEqual(["command"]);
    expect(parsed.draftclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.draftclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"draftclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"draftclaw": {"events": ["test"]}}');
  });
...
describe("resolveDraftClawMetadata", () => {
  it("extracts draftclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveDraftClawMetadata(frontmatter);
  ...
  it("returns undefined when draftclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        draftclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with DraftClaw" },
            { id: "npm", kind: "npm", package: "@draftclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@draftclaw/hook");
  });
    ...
    "draftclaw":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with DraftClaw" }],
      },
  }
---
...
`;
    ...
    const draftclaw = resolveDraftClawMetadata(frontmatter);
    expect(draftclaw).toBeDefined();
    expect(draftclaw?.emoji).toBe("💾");
    expect(draftclaw?.events).toEqual(["command:new"]);
    expect(draftclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(draftclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const draftclaw = resolveDraftClawMetadata(frontmatter);
    expect(draftclaw?.emoji).toBe("disk");
    expect(draftclaw?.events).toEqual(["command:new"]);
  });
});
