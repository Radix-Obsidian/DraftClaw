...
const saveSessionToMemory: HookHandler = async (event) => {
  ...
  try {
    ...
    const cfg = context.cfg as DraftClawConfig | undefined;
    const agentId = resolveAgentIdFromSessionKey(event.sessionKey);
    const workspaceDir = cfg
      ? resolveAgentWorkspaceDir(cfg, agentId)
      : path.join(os.homedir(), ".draftclaw", "workspace");
    ...
    if (sessionFile) {
      ...
      if (sessionContent && cfg) {
        ...
        const draftclawRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
        const slugGenPath = path.join(draftclawRoot, "llm-slug-generator.js");
