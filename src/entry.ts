...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.DRAFTCLAW_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.DRAFTCLAW_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.DRAFTCLAW_NODE_OPTIONS_READY = "1";
  ...
  child.once("error", (error) => {
    console.error(
      "[draftclaw] Failed to respawn CLI:",
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[draftclaw] ${parsed.error}`);
    process.exit(2);
  }
