...
const execHostEnforced = process.env.DRAFTCLAW_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.DRAFTCLAW_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureDraftClawCliOnPath({ pathEnv: process.env.PATH ?? "" });
