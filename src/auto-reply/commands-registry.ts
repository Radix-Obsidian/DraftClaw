...
import type { DraftClawConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: DraftClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: DraftClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as DraftClawConfig),
...
export function getCommandDetection(_cfg?: DraftClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: DraftClawConfig) {
