...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.DRAFTCLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.DRAFTCLAW_SKIP_GMAIL_WATCHER;
    process.env.DRAFTCLAW_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.DRAFTCLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.DRAFTCLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.DRAFTCLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
