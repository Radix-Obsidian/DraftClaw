    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "draftclaw", messages: [{ role: "user", content: "hi" }] },
          { "x-draftclaw-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-draftclaw-agent-id": "beta",
            "x-draftclaw-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
