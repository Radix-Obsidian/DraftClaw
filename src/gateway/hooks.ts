...
export function resolveHooksConfig(cfg: DraftClawConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-draftclaw-token"] === "string"
      ? req.headers["x-draftclaw-token"].trim()
      : "";
  if (headerToken) {
