...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `draftclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.DRAFTCLAW_CONFIG_PATH = path.join(root, "draftclaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "draftclaw.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "draftclaw-gateway-test"),
        ...fileDefaults,
