...
import { discoverDraftClawPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: DraftClawPluginDefinition;
  register?: DraftClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as DraftClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as DraftClawPluginDefinition;
...
export function loadDraftClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverDraftClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "draftclaw/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: DraftClawPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as DraftClawPluginModule;
    } catch (err) {
