...
import { loadDraftClawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.DRAFTCLAW_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `draftclaw-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "draftclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.DRAFTCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadDraftClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.DRAFTCLAW_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadDraftClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadDraftClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
