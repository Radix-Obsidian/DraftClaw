...
function setWhatsAppDmPolicy(cfg: DraftClawConfig, dmPolicy: DmPolicy): DraftClawConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: DraftClawConfig, allowFrom?: string[]): DraftClawConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: DraftClawConfig, selfChatMode: boolean): DraftClawConfig {
...
async function detectWhatsAppLinked(cfg: DraftClawConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so DraftClaw can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for DraftClaw" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("draftclaw channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
