...
function setSlackDmPolicy(cfg: DraftClawConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "DraftClaw";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for DraftClaw`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: DraftClawConfig, allowFrom: string[]): DraftClawConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
