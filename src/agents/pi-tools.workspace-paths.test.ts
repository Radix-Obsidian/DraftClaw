...
import { createDraftClawCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("draftclaw-ws-", async (workspaceDir) => {
      await withTempDir("draftclaw-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createDraftClawCodingTools({ workspaceDir });
    ...
    await withTempDir("draftclaw-ws-", async (workspaceDir) => {
      await withTempDir("draftclaw-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello draftclaw");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("draftclaw-ws-", async (workspaceDir) => {
      await withTempDir("draftclaw-override-", async (overrideDir) => {
        const tools = createDraftClawCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("draftclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("draftclaw-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "draftclaw-sbx-test",
        ...
        const tools = createDraftClawCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
