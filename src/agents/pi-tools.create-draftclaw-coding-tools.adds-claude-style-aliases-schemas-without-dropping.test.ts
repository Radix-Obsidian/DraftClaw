...
import { __testing, createDraftClawCodingTools } from "./pi-tools.js";
...
describe("createDraftClawCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createDraftClawTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createDraftClawCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "draftclaw-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "draftclaw-outside.txt");
