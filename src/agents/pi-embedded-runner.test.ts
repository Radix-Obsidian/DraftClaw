...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "draftclaw-embedded-agent-"));
...
const ensureModels = (cfg: DraftClawConfig) => ensureDraftClawModelsJson(cfg, agentDir) as unknown;
