...
function resolveExecConfig(cfg: DraftClawConfig | undefined) {
...
export function createDraftClawCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createDraftClawReadTool(freshReadTool)];
    }
