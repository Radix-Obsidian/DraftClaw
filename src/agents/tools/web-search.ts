...
type WebSearchConfig = NonNullable<DraftClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: DraftClawConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("draftclaw configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.draftclaw.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://draftclaw.ai",
      "X-Title": "DraftClaw Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
