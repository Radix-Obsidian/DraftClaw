...
/**
 * Cleanup suspended DraftClaw CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: DraftClawConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "draftclaw-cli-images-"));
  const paths: string[] = [];
