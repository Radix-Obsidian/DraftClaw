...
const defaultTools = createDraftClawCodingTools();

describe("createDraftClawCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "draftclaw-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createDraftClawCodingTools();
    ...
    try {
      ...
      const contents = "Hello from draftclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "draftclaw-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "draftclaw-workspace"),
    ...
    const tools = createDraftClawCodingTools({ sandbox });
