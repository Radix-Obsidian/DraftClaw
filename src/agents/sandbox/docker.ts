...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "draftclaw.sandbox=1");
  args.push("--label", `draftclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `draftclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `draftclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("draftclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`draftclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`draftclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("draftclaw sandbox recreate --all");
}
