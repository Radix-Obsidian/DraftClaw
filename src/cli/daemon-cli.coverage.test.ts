...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    DRAFTCLAW_STATE_DIR: process.env.DRAFTCLAW_STATE_DIR,
    DRAFTCLAW_CONFIG_PATH: process.env.DRAFTCLAW_CONFIG_PATH,
    DRAFTCLAW_GATEWAY_PORT: process.env.DRAFTCLAW_GATEWAY_PORT,
    DRAFTCLAW_PROFILE: process.env.DRAFTCLAW_PROFILE,
  };

  beforeEach(() => {
    process.env.DRAFTCLAW_STATE_DIR = "/tmp/draftclaw-cli-state";
    process.env.DRAFTCLAW_CONFIG_PATH = "/tmp/draftclaw-cli-state/draftclaw.json";
    delete process.env.DRAFTCLAW_GATEWAY_PORT;
    delete process.env.DRAFTCLAW_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.DRAFTCLAW_STATE_DIR !== undefined) {
      process.env.DRAFTCLAW_STATE_DIR = originalEnv.DRAFTCLAW_STATE_DIR;
    } else {
      delete process.env.DRAFTCLAW_STATE_DIR;
    }

    if (originalEnv.DRAFTCLAW_CONFIG_PATH !== undefined) {
      process.env.DRAFTCLAW_CONFIG_PATH = originalEnv.DRAFTCLAW_CONFIG_PATH;
    } else {
      delete process.env.DRAFTCLAW_CONFIG_PATH;
    }

    if (originalEnv.DRAFTCLAW_GATEWAY_PORT !== undefined) {
      process.env.DRAFTCLAW_GATEWAY_PORT = originalEnv.DRAFTCLAW_GATEWAY_PORT;
    } else {
      delete process.env.DRAFTCLAW_GATEWAY_PORT;
    }

    if (originalEnv.DRAFTCLAW_PROFILE !== undefined) {
      process.env.DRAFTCLAW_PROFILE = originalEnv.DRAFTCLAW_PROFILE;
    } else {
      delete process.env.DRAFTCLAW_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        DRAFTCLAW_PROFILE: "dev",
        DRAFTCLAW_STATE_DIR: "/tmp/draftclaw-daemon-state",
        DRAFTCLAW_CONFIG_PATH: "/tmp/draftclaw-daemon-state/draftclaw.json",
        DRAFTCLAW_GATEWAY_PORT: "19001",
      },
