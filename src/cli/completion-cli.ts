...
export async function installCompletion(shell: string, yes: boolean, binName = "draftclaw") {
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c draftclaw -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'draftclaw gateway' and not 'draftclaw other gateway', we need to check the sequence.
