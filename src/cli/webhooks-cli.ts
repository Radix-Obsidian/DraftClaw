...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.draftclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + DraftClaw hooks")
    ...
    .option("--hook-url <url>", "DraftClaw hook URL")
    .option("--hook-token <token>", "DraftClaw hook token")
