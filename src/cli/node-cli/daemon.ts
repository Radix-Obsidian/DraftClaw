...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("draftclaw node install"), formatCliCommand("draftclaw node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("draftclaw node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    DRAFTCLAW_LOG_PREFIX: baseEnv.DRAFTCLAW_LOG_PREFIX ?? "node",
