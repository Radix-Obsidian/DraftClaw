...
${formatHelpExamples([
  ['draftclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['draftclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'draftclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'draftclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['draftclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'draftclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.draftclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.draftclaw.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['draftclaw agents set-identity --agent main --name "DraftClaw" --emoji "🦞"', "Set name + emoji."],
  ["draftclaw agents set-identity --agent main --avatar avatars/draftclaw.png", "Set avatar path."],
  [
    "draftclaw agents set-identity --workspace ~/.draftclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "draftclaw agents set-identity --identity-file ~/.draftclaw/workspace/IDENTITY.md --agent main",
