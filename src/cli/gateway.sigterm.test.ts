...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "draftclaw-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      DRAFTCLAW_NO_RESPAWN: "1",
      DRAFTCLAW_STATE_DIR: stateDir,
      DRAFTCLAW_SKIP_CHANNELS: "1",
      DRAFTCLAW_SKIP_GMAIL_WATCHER: "1",
      DRAFTCLAW_SKIP_CRON: "1",
      DRAFTCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
      DRAFTCLAW_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "draftclaw-entry-bootstrap.mjs");
