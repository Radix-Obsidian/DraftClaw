...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.draftclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.DRAFTCLAW_PROFILE = profile;

  const stateDir = env.DRAFTCLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.DRAFTCLAW_STATE_DIR?.trim()) {
    env.DRAFTCLAW_STATE_DIR = stateDir;
  }

  if (!env.DRAFTCLAW_CONFIG_PATH?.trim()) {
    env.DRAFTCLAW_CONFIG_PATH = path.join(stateDir, "draftclaw.json");
  }

  if (profile === "dev" && !env.DRAFTCLAW_GATEWAY_PORT?.trim()) {
    env.DRAFTCLAW_GATEWAY_PORT = "19001";
  }
}
