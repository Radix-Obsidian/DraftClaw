...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "draftclaw-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "draftclaw doctor",
        managerScriptArgs(manager, "draftclaw", ["doctor", "--non-interactive"]),
        gitRoot,
        { DRAFTCLAW_UPDATE_IN_PROGRESS: "1" },
      ),
    );
