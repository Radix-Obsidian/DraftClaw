...
export function triggerDraftClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.DRAFTCLAW_SYSTEMD_UNIT,
        process.env.DRAFTCLAW_PROFILE,
      );
  ...
  const label =
    process.env.DRAFTCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.DRAFTCLAW_PROFILE);
