...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.DRAFTCLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*draftclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "draftclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "draftclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "draftclaw-gw");
  ...
  records.push(`_draftclaw-gw._tcp IN PTR ${instanceLabel}._draftclaw-gw._tcp`);
  records.push(`${instanceLabel}._draftclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._draftclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; draftclaw-content-hash: ${contentHash}\n${contentBody}`;
}
