...
function resolveActiveHoursTimezone(cfg: DraftClawConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: DraftClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: DraftClawConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: DraftClawConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: DraftClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: DraftClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: DraftClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: DraftClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
