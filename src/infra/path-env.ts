...
function candidateBinDirs(opts: EnsureDraftClawPathOpts): string[] {
  ...
  // Bundled macOS app: `draftclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "draftclaw");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/draftclaw` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "draftclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `draftclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureDraftClawCliOnPath(opts: EnsureDraftClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.DRAFTCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.DRAFTCLAW_PATH_BOOTSTRAPPED = "1";
