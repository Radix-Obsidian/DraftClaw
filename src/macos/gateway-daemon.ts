...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `draftclaw --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.DRAFTCLAW_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.DRAFTCLAW_GATEWAY_BIND ??
...
void main().catch((err) => {
  console.error(
    "[draftclaw] Gateway daemon failed:",
