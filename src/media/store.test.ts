...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "DRAFTCLAW_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "draftclaw-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.DRAFTCLAW_STATE_DIR = path.join(home, ".draftclaw");
    ...
    await fs.mkdir(path.join(home, ".draftclaw"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.draftclaw${path.sep}media`);
