import type { BrowserConfig, BrowserProfileConfig, DraftClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_DRAFTCLAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_DRAFTCLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "draftclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_DRAFTCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_DRAFTCLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an DraftClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for draftclaw/draftclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: DraftClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_DRAFTCLAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "draftclaw";
