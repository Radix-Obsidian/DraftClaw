...
vi.mock("./chrome.js", () => ({
  resolveDraftClawUserDataDir: vi.fn(() => "/tmp/draftclaw-test/draftclaw/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveDraftClawUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "draftclaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveDraftClawUserDataDir).mockReturnValue(userDataDir);
