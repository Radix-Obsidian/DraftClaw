...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".draftclaw-profile-decorated");
}
 ...
 */
export function decorateDraftClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_DRAFTCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_DRAFTCLAW_BROWSER_COLOR).toUpperCase();
