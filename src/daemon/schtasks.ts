...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.DRAFTCLAW_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.DRAFTCLAW_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.DRAFTCLAW_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.DRAFTCLAW_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
