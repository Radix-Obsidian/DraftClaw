...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.DRAFTCLAW_GATEWAY_PORT).toBe("18789");
    expect(env.DRAFTCLAW_GATEWAY_TOKEN).toBe("secret");
    expect(env.DRAFTCLAW_SERVICE_MARKER).toBe("draftclaw");
    expect(env.DRAFTCLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.DRAFTCLAW_SERVICE_VERSION).toBe("string");
    expect(env.DRAFTCLAW_SYSTEMD_UNIT).toBe("draftclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.DRAFTCLAW_LAUNCHD_LABEL).toBe("ai.draftclaw.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", DRAFTCLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.DRAFTCLAW_SYSTEMD_UNIT).toBe("draftclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.DRAFTCLAW_LAUNCHD_LABEL).toBe("ai.draftclaw.work");
    }
  });
});
