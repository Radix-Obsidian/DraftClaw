...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.DRAFTCLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.DRAFTCLAW_STATE_DIR;
  const configPath = env.DRAFTCLAW_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    DRAFTCLAW_PROFILE: profile,
    DRAFTCLAW_STATE_DIR: stateDir,
    DRAFTCLAW_CONFIG_PATH: configPath,
    DRAFTCLAW_GATEWAY_PORT: String(port),
    DRAFTCLAW_GATEWAY_TOKEN: token,
    DRAFTCLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    DRAFTCLAW_SYSTEMD_UNIT: systemdUnit,
    DRAFTCLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    DRAFTCLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    DRAFTCLAW_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    DRAFTCLAW_STATE_DIR: stateDir,
    DRAFTCLAW_CONFIG_PATH: configPath,
    DRAFTCLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    DRAFTCLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    DRAFTCLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    DRAFTCLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    DRAFTCLAW_LOG_PREFIX: "node",
    DRAFTCLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    DRAFTCLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    DRAFTCLAW_SERVICE_VERSION: VERSION,
  };
}
