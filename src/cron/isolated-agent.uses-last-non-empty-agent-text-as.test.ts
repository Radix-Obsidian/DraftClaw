...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "draftclaw-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".draftclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<DraftClawConfig> = {},
): DraftClawConfig {
  const base: DraftClawConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "draftclaw"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".draftclaw", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
