import {
  ...
  type DraftClawConfig,
  type RuntimeEnv,
} from "draftclaw/plugin-sdk";
...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as DraftClawConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as DraftClawConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as DraftClawConfig);
