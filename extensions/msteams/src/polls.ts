...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["draftclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["draftclaw", "pollId"]) ??
    readNestedString(value, ["draftclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "draftclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "draftclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          draftclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "draftclaw poll vote",
          displayText: "Vote recorded",
          value: { draftclawPollId: pollId, pollId },
        },
      },
    ],
  };
