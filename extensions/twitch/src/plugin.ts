/**
 * Twitch channel plugin for DraftClaw.
 *
...
import { buildChannelConfigSchema } from "draftclaw/plugin-sdk";
import type {
 ...
 * for DraftClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: DraftClawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: DraftClawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: DraftClawConfig): boolean => {
