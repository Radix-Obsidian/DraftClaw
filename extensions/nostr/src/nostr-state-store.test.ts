...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.DRAFTCLAW_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "draftclaw-nostr-"));
  process.env.DRAFTCLAW_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.DRAFTCLAW_STATE_DIR?.trim() || env.DRAFTCLAW_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".draftclaw");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.DRAFTCLAW_STATE_DIR = previous;
    }
