...
// NOTE: This extension is intended to be bundled with DraftClaw.
// When running from source (tests/dev), DraftClaw internals live under src/.
...
export function createLlmTaskTool(api: DraftClawPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via draftclaw.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "draftclaw-llm-task-"));
