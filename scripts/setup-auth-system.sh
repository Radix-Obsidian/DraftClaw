#!/bin/bash
# Setup DraftClaw Auth Management System
...
echo "=== DraftClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'draftclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  DraftClaw message: Send warning via DraftClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/draftclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/draftclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/draftclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now draftclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status draftclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
