...
#
# Suggested cron: */30 * * * * /home/admin/draftclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send DraftClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., draftclaw-alerts)
...
STATE_FILE="$HOME/.draftclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via DraftClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use draftclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via DraftClaw to $NOTIFY_PHONE..."
            draftclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! DraftClaw is down. Run: ssh l36 '~/draftclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
