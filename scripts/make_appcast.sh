...
ZIP=${1:?"Usage: $0 DraftClaw-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/draftclaw/draftclaw/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^DraftClaw-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/draftclaw/draftclaw/releases/download/v${VERSION}/"}
