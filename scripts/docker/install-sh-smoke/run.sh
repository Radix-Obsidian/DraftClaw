...
INSTALL_URL="${DRAFTCLAW_INSTALL_URL:-https://draftclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${DRAFTCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${DRAFTCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="draftclaw"
PACKAGE_NAME="${DRAFTCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (DRAFTCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${DRAFTCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${DRAFTCLAW_INSTALL_LATEST_OUT:-}"
fi
