...
IMAGE_NAME="draftclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="draftclaw-net-e2e-$$"
GW_NAME="draftclaw-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "DRAFTCLAW_GATEWAY_TOKEN=$TOKEN" \
	  -e "DRAFTCLAW_SKIP_CHANNELS=1" \
	  -e "DRAFTCLAW_SKIP_GMAIL_WATCHER=1" \
	  -e "DRAFTCLAW_SKIP_CRON=1" \
	  -e "DRAFTCLAW_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
