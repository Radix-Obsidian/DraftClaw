...
IMAGE_NAME="draftclaw-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/draftclaw-bin:$PATH"
  mkdir -p /tmp/draftclaw-bin

  cat > /tmp/draftclaw-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/draftclaw-bin/systemctl

  cat > /tmp/draftclaw-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/draftclaw-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/draftclaw"
  npm_entry="/tmp/npm-prefix/lib/node_modules/draftclaw/dist/index.js"
  git_entry="/app/dist/index.js"
  git_cli="/app/draftclaw.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/draftclaw-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/draftclaw-gateway.service"
    if [ ! -f "$unit_path" ]; then
