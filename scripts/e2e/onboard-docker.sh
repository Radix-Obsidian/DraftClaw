...
IMAGE_NAME="draftclaw-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/draftclaw-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/draftclaw-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/draftclaw-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/draftclaw-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/draftclaw-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.draftclaw/workspace"
    config_path="$HOME/.draftclaw/draftclaw.json"
    sessions_dir="$HOME/.draftclaw/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.draftclaw/draftclaw.json" <<'"'"'JSON'"'"'
{
