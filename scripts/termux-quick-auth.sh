...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/draftclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "DraftClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/draftclaw/scripts/mobile-reauth.sh'" --id draftclaw-auth
        ;;
    *)
