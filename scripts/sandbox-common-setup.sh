...
BASE_IMAGE="${BASE_IMAGE:-draftclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-draftclaw-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=draftclaw.sandbox=1)
NOTE
